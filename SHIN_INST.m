...
%	REF2	= string - argument describing how the averaging period is
%	referenced to TIME2
...
if size(TIME1,2) == 7
	EL1 = SolarGeometry_v2(TIME1,lat,lon,tz);			% Solar geometry for fine time step
	TIME1 = TIME1(:,7);									% Serial dates
else
	TIME1 = time_builder(TIME1);						% SolarGeometry requires time_builder matrix
	EL1 = SolarGeometry_v2(TIME1,lat,lon,tz);			% Solar geometry for fine time step
	TIME1 = TIME1(:,7);									% Serial dates
end
...
TAU1 = Trans_INST(SW1,EL1);							% Transmissivity @ TIME1 using average SZA
...
TAU_fine = interp1(TIME1,TAU1,t_fine(:,7));
